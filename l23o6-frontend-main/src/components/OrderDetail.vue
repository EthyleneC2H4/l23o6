...
const payMethod = (method : number, id: number) =>{
  request({
    url:`/order/${id}/${method}`,
    method:'PUT'
  })
}
const pay1 = (id: number) => {
  window.location.href = "https://excashier-sandbox.dl.alipaydev.com/standard/auth.htm?payOrderId=dc43390bf3c34a2eb04de0cde0876b3b.00"
  request({
    url: `/order/${id}`,
    method: 'PATCH',
    data: {
  ...
  <div style="display: flex; flex-direction: column">
    ...
    <div style="margin-top: 2vh" v-if="orderDetail.data && orderDetail.data.status === '等待支付'">
      <div style="float:right;">
        ...
        <el-button type="primary" @click="payMethod(2, id ?? -1);pay(id ?? -1)">
          积分支付
        </el-button>
        <el-button type="primary" @click="payMethod( 1,id ?? -1);pay1(id ?? -1)">
          支付宝支付
        </el-button>
      </div>
    </div>
