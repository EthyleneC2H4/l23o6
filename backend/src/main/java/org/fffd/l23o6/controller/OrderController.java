...
public class OrderController {
    ...
    public Long getMethod(@PathVariable Long method, @PathVariable("orderId") Long orderId){
        System.out.println("first" + method);
        orderService.setMethod(orderId, method + "");
        return method;
    }
    ...
    public CommonResponse<?> patchOrder(@PathVariable("orderId") Long orderId, @Valid @RequestBody PatchOrderRequest request) {
        ...
        switch (request.getStatus()) {
            case PAID:
                StpUtil.checkLogin();
                orderService.payOrder(orderId, StpUtil.getLoginIdAsString());
                break;
            case CANCELLED:
